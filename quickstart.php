               ...
               $index=0;
                if ($result->num_rows > 0) {
                 ...
                 while($row = $result->fetch_assoc()) {
                    ...
                    $sql3 = "SELECT email FROM `actor`\n"

                    . "INNER JOIN actors_in_plays ON actors_in_plays.id=actor.id\n"

                    . "WHERE actors_in_plays.playName=\"$name\"";
                     ...
                     while ($index<count($emails))
                   {
                       $send=false;
                       $index2=0;
                       while($index2<count($s_emails))
                       {
                    //check if the actor in play than send him the shows that he participate them
                          if ($emails[$index]==$s_emails[$index2])
                             {
                               $send==true;
                             }
